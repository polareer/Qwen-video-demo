...
from .schemas import ErgonomicMetrics, ErgonomicRiskEvent, Evidence, QwenExplanation, TargetState, TaskState
...
class ErgonomicAnalyzer:
    ...
    def _process_window(
        ...
        task_windows.append(self._task_window(task_state, window))
        event = self._event_from_window(video_id, window, len(events) + 1, last_event_time, task_state)
        if event is None:
            return
    ...
    def _event_from_window(
        ...
        risk_type = self._risk_type(analyses)
    ...
    def _risk_type(self, analyses: list[FrameAnalysis]) -> str | None:
    ...
    def _local_explanation(self, event: ErgonomicRiskEvent) -> QwenExplanation:
        ...
        return QwenExplanation(
            ...
            evidence_description="该结论来自本地视觉规则引擎，未调用 Qwen2.5-VL。",
            needs_human_review=event.risk_level == "high",
        )

    def _task_window(self, task_state: TaskState, window: list[tuple[FrameAnalysis, Any]]) -> dict[str, Any]:
    ...
    def _risk_level(self, metrics: ErgonomicMetrics) -> str:
